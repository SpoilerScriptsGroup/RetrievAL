...
descriptor_record struct                                ; record for table of cache descriptors
    d_key   byte ?                                      ; key from cpuid instruction
    d_level byte ?                                      ; cache level
    d_sizem byte ?                                      ; size multiplier
    d_2pow  byte ?                                      ; power of 2. size = d_sizem << d_2pow
...
data_layout struct
    ...
    level3          dword             ?
    level4          dword             ?
    descriptortable descriptor_record 60 dup (<?>)
data_layout ends

.data

dataref label data_layout                               ; reference point
...
descriptortable_ label descriptor_record                ; table of Intel cache descriptors
...
DataCacheSize proc near
	...
	push    edi
	push    ebp
	mov     edi, dword ptr [esp + 20]                   ; level
	mov     esi, offset dataref                         ; point to dataref

	assume esi:ptr data_layout
...
D800:
	...
	; level = 1 .. numlevels
	mov     eax, dword ptr [esi + edi * 4]              ; size of selected cache
	jmp     D850
...
D850:
	mov     [esi].ok, 1                                 ; remember called, whether success or not
...
I180:
	mov     dword ptr [esi + eax * 4], ecx              ; store size of data cache level eax
...
J200:
	cmp     al, [esi].descriptortable[ebx * sizeof(descriptor_record)].d_key
	jne     J300

	; descriptor found
	;       YASM v. 1.1.0 fails if there are too many of (label-dataref): !
	;       movzx   eax, byte [esi + ebx*4 + (descriptortable_-dataref) + d_sizem]
	movzx   eax, [esi].descriptortable[ebx * sizeof(descriptor_record)].d_sizem
	mov     cl,  [esi].descriptortable[ebx * sizeof(descriptor_record)].d_2pow
	shl     eax, cl                                     ; compute size
	movzx   ecx, [esi].descriptortable[ebx * sizeof(descriptor_record)].d_level

	; check that level = 1-3
	cmp     ecx, 3
	ja      J300
	mov     dword ptr [esi + ecx * 4], eax              ; store size eax of data cache level ecx
...
K100:
	mov     [esi].level3, ecx                           ; store L3 data cache size
