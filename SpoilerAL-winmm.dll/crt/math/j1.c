...
#define GET_LOW_WORD(lsw, x)       (lsw = ((uint32_t *)&(x))[0])
#define GET_HIGH_WORD(msw, x)      (msw = ((uint32_t *)&(x))[1])
#define EXTRACT_WORDS(msw, lsw, x) do { GET_LOW_WORD(lsw, x); GET_HIGH_WORD(msw, x); } while (0)

static const uint64_t sNaN = 0xFFF0000000000000;
#define SNAN (*(double *)&sNaN)
...
double __cdecl y1(double x)
{
	...
	return x * (u / v) + tpi * (_j1(x) * log(x) - 1 / x);
}
