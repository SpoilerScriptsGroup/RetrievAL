...
wchar_t * __vectorcall internal_wmemrchrAVX2(const wchar_t *buffer, __m128 c, size_t count);
...
__declspec(naked) wchar_t * __vectorcall internal_wmemrchrAVX2(const wchar_t *buffer, __m128 c, size_t count)
{
	__asm
	{
		#define buffer ecx
...
__declspec(naked) static wchar_t * __cdecl wmemrchrCPUDispatch(const wchar_t *buffer, wchar_t c, size_t count)
{
	...
	__asm
	{
		...
		jb      L1
		mov     dword ptr [wmemrchrDispatch], offset wmemrchrAVX2
		jmp     wmemrchrAVX2

	L1:
		cmp     eax, __ISA_AVAILABLE_SSE2
		jbe     L2
		mov     dword ptr [wmemrchrDispatch], offset wmemrchrSSE42
		jmp     wmemrchrSSE42
