...
#pragma warning(disable: 4163 4615)
#endif

#pragma function(memcpy, memset, strlen, log10, ceil)
...
//
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
__forceinline static uint32_t __acrt_fp_classify(const double *value)
{
...
__forceinline static errno_t __acrt_fp_strflt_to_string(
	char         *buffer,
	...
	const size_t   mantissa_buffer_count)
{
	...
	assert(mantissa_buffer_count > 0);
	...
	{
		assert(("unexpected input value; log10 failed", 0));
		k = 0;
	}
	...
	*exponent = k; // k is now correct; store it for our caller
	...
	for (;;)
	{
		...
		assert(quotient < digits_per_iteration_multiplier);
...
// those functions.
__forceinline static errno_t __acrt_fp_format(
	const double   *value,
	char           *result_buffer,
	const size_t   result_buffer_count,
	char           *scratch_buffer,
