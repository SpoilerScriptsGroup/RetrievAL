...
__declspec(naked) uint32_t __msfastcall fnv1a32(const void *data, size_t size)
{
	__asm
	{
		...
		push    prime                           // 00000001 _ 68, 811C9DC5
...
#endif
{
	__asm
	{
		...
		ret                                     // 0000000F _ C3
#else
		mov     r0, dword ptr [hash]            // 00000000 _ 8B. 44 24, 0C
...
__declspec(naked) uint64_t __msreturn __msfastcall fnv1a64(const void *data, size_t size)
{
	__asm
	{
		...
		pop     eax                             // 00000000 _ 58
		push    basis_hi                        // 00000001 _ 68, CBF29CE4
		push    basis_lo                        // 00000006 _ 68, 84222325
		push    eax                             // 0000000B _ 50
		jmp     fnv1a64combine                  // 0000000C _ E9, ????????
