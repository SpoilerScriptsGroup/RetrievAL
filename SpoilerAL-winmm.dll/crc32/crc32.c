...
#define POLYNOMIAL32 0xEDB88320U
#define MAX_SLICE 4
...
DWORD CRC32FromFileHandle(IN HANDLE hFile)
{
	...
	if ((nBufferSize = GetFileSize(hFile, NULL)) <= sizeof(lpStackBuffer) ||
		(hHeap = GetProcessHeap()) == NULL ||
		(lpBuffer = (LPBYTE)HeapAlloc(hHeap, 0, nBufferSize = min(nBufferSize, CRC32_FILEBUFFER_SIZE))) == NULL)
	{
		hHeap = NULL;
