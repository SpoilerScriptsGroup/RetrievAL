...
#ifndef _M_IX86
int __cdecl _mbsnbcmp(const unsigned char *string1, const unsigned char *string2, size_t count)
{
	int ret;
	...
	ret = CompareStringA(GetThreadLocale(), 0, string1, count, string2, count);
	__assume(ret >= 0 && ret <= 3);
	return ret ? ret - CSTR_EQUAL : _NLSCMPERROR;
}
#else
__declspec(naked) int __cdecl _mbsnbcmp(const unsigned char *string1, const unsigned char *string2, size_t count)
{
	__asm
	{
