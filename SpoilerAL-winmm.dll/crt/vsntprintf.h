...
int __fastcall internal_vsntprintf(TCHAR *buffer, size_t count, const TCHAR *format, va_list argptr, const va_list endarg)
{
	...
	#define va_try_increase(argptr, endarg, type) (!(endarg) ? (argptr = (va_prev = argptr) + va_sizeof(type), true) : !_add_u32((uint32_t)(va_prev = argptr), va_sizeof(type), (uint32_t *)&argptr) ? argptr <= endarg : (argptr = (va_list)-1, false))
	...
	#define va_try_increase(argptr, endarg, type) (!(endarg) ? (argptr = (va_prev = argptr) + va_sizeof(type), true) : !_add_u64((uint64_t)(va_prev = argptr), va_sizeof(type), (uint64_t *)&argptr) ? argptr <= endarg : (argptr = (va_list)-1, false))
	#define va_read(argptr, endarg, type, default) (va_try_increase(argptr, endarg, type) ? va_indirection(va_prev, type) : (type)(default))
#else
	#define va_indirection va_arg
	#define va_try_increase(argptr, endarg, type) (va_prev = argptr, va_arg(argptr, type), !(endarg) || (argptr > va_prev ? argptr <= endarg : (argptr = (va_list)-1, false)))
	#define va_read(argptr, endarg, type, default) (va_try_increase(argptr, endarg, type) ? va_indirection(va_prev, type) : (type)(default))
#endif
